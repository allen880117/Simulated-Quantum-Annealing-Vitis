...
void Run(const u32_t stage, const info_t info, const state_t state,
         spin_pack_t trotters_local[NUM_SPIN / PACKET_SIZE], fp_pack_strm_t& j_stream) {
...
#pragma HLS UNROLL
                ...
                fp_buffer[strm_ofst][spin_ofst] = Multiply(
                    trotters_local[pack_ofst + strm_ofst][spin_ofst], tmp_pack.data[spin_ofst]);
            }
...
#pragma HLS UNROLL
    ...
    void ReadTrottersFromHost(spin_pack_t trotters[NUM_TROT][NUM_SPIN / PACKET_SIZE]) {
...
#pragma HLS PIPELINE
    ...
    void WriteTrottersToHost(spin_pack_t trotters[NUM_TROT][NUM_SPIN / PACKET_SIZE]) {
    WRITE_TROTTERS:
        for (u32_t t = 0; t < NUM_TROT; t++) {
        WRITE_TROTTERS_1:
            for (u32_t ofst = 0; ofst < NUM_SPIN / PACKET_SIZE; ofst++) {
#pragma HLS PIPELINE
    ...
    void UpdateJcoup(const u32_t stage, const fp_pack_t jcoup[NUM_SPIN][NUM_SPIN / PACKET_SIZE],
...
#pragma HLS INLINE OFF
    ...
    READ_JCOUP:
        for (u32_t ofst = 0; ofst < NUM_SPIN / PACKET_SIZE; ofst++) {
#pragma HLS PIPELINE
            fp_pack_t tmp_pack   = jcoup[stage & (NUM_SPIN - 1)][ofst];
            jcoup_local[0][ofst] = tmp_pack;
            j_stream[0] << tmp_pack;
        }
        // }

        this->ShiftJcoupCache(j_stream);
    }

    void ShiftJcoupCache(fp_pack_strm_t j_stream[NUM_TROT]) {
#pragma HLS INLINE OFF
    SHIFT_JCOUP:
        for (u32_t ofst = 0; ofst < NUM_SPIN / PACKET_SIZE; ofst++) {
#pragma HLS PIPELINE
...
#pragma HLS UNROLL
                j_stream[t + 1] << jcoup_local[t + 1][ofst];
...
LOOP_STAGE:
    for (u32_t stage = 0; stage < (NUM_SPIN + NUM_TROT - 1); stage++) {
        ...
        // Update jcoup
        cu.UpdateJcoup(stage, jcoup, j_stream);

        // Run Trotter Units
    RUN_TU:
        for (u32_t t = 0; t < NUM_TROT; t++) {
#pragma HLS UNROLL
            TrotterUnit::Run(stage, info[t], cu.state[t], cu.trotters_local[t], j_stream[t]);
        }
    }
